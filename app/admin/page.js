...
function Badge({ children, color }) {
  const styles = {
    green: "bg-emerald-50 text-emerald-600 border-emerald-100 dark:bg-emerald-950/30 dark:text-emerald-400 dark:border-emerald-800/50",
    red: "bg-red-50 text-red-600 border-red-100 dark:bg-red-950/30 dark:text-red-400 dark:border-red-800/50",
    purple: "bg-zinc-100 text-zinc-600 border-zinc-200 dark:bg-zinc-800 dark:text-zinc-400 dark:border-zinc-700",
    gray: "bg-muted/50 text-muted-foreground border-border",
  };
  return (
    <span
      className={`text-[10px] px-2 py-0.5 rounded-md font-bold uppercase tracking-wider border ${styles[color] || styles.gray}`}
    >
      {children}
    </span>
  );
    ...
    return (
      <div className="flex items-center gap-4 py-4 border-b border-zinc-100 dark:border-zinc-800/50 last:border-0 group">
        ...
        <div className="relative">
          <Avatar user={member} className="w-11 h-11 shadow-sm ring-1 ring-zinc-100 dark:ring-zinc-800" />
          {!isActive && (
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-0.5">
            ...
            {isAdmin && <Crown className="w-3 h-3 text-amber-500 fill-amber-500/20" />}
          </div>
            ...
            {isActive ? (
              <>
                ...
                <button
                  ...
                  title={isAdmin ? "Hapus Admin" : "Jadikan Admin"}
                >
                  {isAdmin ? <ShieldOff className="w-4.5 h-4.5" /> : <Shield className="w-4.5 h-4.5" />}
                </button>
    ...
    <div className="min-h-screen bg-muted/20 pb-12">
        ...
        <div className="max-w-3xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <h1 className="font-bold text-sm uppercase tracking-widest text-foreground">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: 'Total', count: members.length, color: 'bg-zinc-200 dark:bg-zinc-800' },
            { label: lang === "id" ? "Aktif" : "Active", count: activeMembers.length, color: 'bg-emerald-500' },
            { label: lang === "id" ? "Pasif" : "Inactive", count: inactiveMembers.length, color: 'bg-red-500' }
          ].map((stat, idx) => (
            <div key={idx} className="bg-background border rounded-2xl p-4 shadow-sm relative overflow-hidden">
              <div className={`absolute top-0 left-0 w-full h-1 ${stat.color} opacity-70`} />
              <p className="text-xl font-bold tabular-nums tracking-tight">{stat.count}</p>
        ...
        <div className="bg-background border rounded-[2rem] overflow-hidden shadow-sm">
          ...
          <div className="p-8 space-y-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 items-start">
              ...
              <div className="space-y-2">
                <label className="text-[11px] font-bold text-muted-foreground/60 uppercase tracking-wider ml-1">
                  {lang === "id" ? "Identity Branding (Logo)" : "Branding Identity"}
                </label>
                <div className="flex items-center gap-5 p-4 bg-muted/5 rounded-[1.5rem] border border-dashed border-border">
                  <div className="w-20 h-20 bg-background border rounded-2xl flex items-center justify-center flex-shrink-0 overflow-hidden shadow-sm">
                    {appSettings?.logo_url ? (
                      <img src={appSettings.logo_url} alt="Logo" className="w-full h-full object-contain p-2" />
                    ) : (
                      <Shield className="w-8 h-8 text-muted-foreground/30" />
                    )}
                  </div>
                  <div className="space-y-3">
                    <label className="cursor-pointer bg-zinc-900 dark:bg-zinc-100 text-zinc-100 dark:text-zinc-900 text-[10px] font-black tracking-widest uppercase px-5 py-2.5 rounded-xl transition-all hover:opacity-90 active:scale-95 flex items-center gap-2 shadow-md">
                      <Upload className="w-3.5 h-3.5" />
                      {lang === "id" ? "PILIH LOGO" : "CHOOSE LOGO"}
                      <input type="file" className="hidden" accept="image/*" onChange={handleUploadLogo} />
                    </label>
                    <p className="text-[9px] text-muted-foreground leading-tight max-w-[120px] font-medium">
                      {lang === "id" ? "Rekomendasi 1:1, Max 2MB." : "Recommended 1:1 ratio, Max 2MB."}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="flex justify-end pt-2">
              <button
                ...
                className="w-full sm:w-auto bg-primary text-primary-foreground text-xs font-black tracking-widest uppercase px-8 py-4 rounded-2xl hover:opacity-90 active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50 transition-all shadow-lg shadow-primary/20"
              >
                {savingSettings ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                {savingSettings ? (lang === "id" ? "MENYIMPAN..." : "SAVING...") : (lang === "id" ? "SIMPAN PERUBAHAN" : "SAVE SETTINGS")}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="bg-background border rounded-[2rem] overflow-hidden shadow-sm">
          <div className="px-6 py-5 border-b bg-muted/10 flex items-center justify-between">
            <h2 className="font-bold text-xs uppercase tracking-widest text-muted-foreground/80">{lang === "id" ? "MEMBER AKTIF" : "ACTIVE MEMBERS"}</h2>
        ...
        {inactiveMembers.length > 0 && (
          <div className="bg-background border rounded-[2rem] overflow-hidden shadow-sm">
            <div className="px-6 py-5 border-b bg-muted/10 flex items-center justify-between">
              <h2 className="font-bold text-xs uppercase tracking-widest text-muted-foreground/80">{lang === "id" ? "AKUN PASIF" : "INACTIVE ACCOUNTS"}</h2>
        ...
        <div className="bg-background border rounded-[2rem] p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-6 shadow-sm overflow-hidden relative group">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-bold tracking-tight">🔔 Test Push Notification</p>
            </div>
            <p className="text-xs text-muted-foreground leading-relaxed max-w-sm">
              {lang === "id" ? "Kirim notifikasi uji coba ke semua member aktif." : "Send a test notification to all active members."}
            </p>
            {testPushMsg && (
              <p className="text-[10px] mt-1 font-black uppercase text-emerald-600 tracking-wider">
                {testPushMsg}
              </p>
            )}
          </div>
          <button
            ...
            className="flex items-center justify-center gap-3 px-6 py-4 bg-muted hover:bg-muted/80 text-foreground rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 flex-shrink-0 shadow-sm border"
          >
            ...
            {[
              {
                ...
                text: lang === "id" ? "Menonaktifkan akun sementara. Data preservasi tetap ada namun hak akses dicabut." : "Deactivates account. Access is revoked but history is preserved."
              },
              {
                title: lang === "id" ? "DELETE" : "DELETE",
                text: lang === "id" ? "Hanya tersedia untuk akun pasif. Menghapus identitas permanen dari database." : "Permanent identity removal from database (only for inactive users)."
              },
              {
                title: lang === "id" ? "ROLES" : "ROLES",
                text: lang === "id" ? "Admin memiliki kontrol penuh atas jadwal dan manajemen tim." : "Admins have full control over schedules and team settings."
              }
            ].map((item, i) => (
              <li key={i} className="flex gap-4">
                <span className="text-[9px] font-black text-white bg-zinc-400 dark:bg-zinc-600 px-1.5 py-0.5 rounded h-fit mt-0.5 w-10 text-center">
                  {item.title}
                </span>
