...
export async function POST(request) {
  try {
    ...
    const limiter = await rateLimitAsync(`notify:${auth.user.id}`, {
      limit: 10,
      windowMs: 60 * 1000,
    });
    if (!limiter.ok) {
      await writeAuditLog({
        action: "notify.rate_limited",
    ...
    const { task, assigneeIds, action } = body;
    ...
    const actor =
      (await auth.userClient
        ...
        .eq("id", auth.user.id)
        .single()).data || null;
    const actorName = actor?.full_name || actor?.email || auth.user.email;
