...
function TypeBadge({ task }) {
  if (task.is_comday || task.task_type === "libur_pengganti") {
    return (
      <span className="inline-flex items-center text-xs bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 rounded-full whitespace-nowrap font-medium">
        🏖️ Libur Pengganti
      </span>
    );
  }
  return (
    <span className="inline-flex items-center text-xs bg-blue-100 dark:bg-blue-500/15 text-blue-700 dark:text-blue-400 px-2 py-0.5 rounded-full whitespace-nowrap font-medium">
      📅 Regular
    </span>
  );
}
...
function TaskRow({ task }) {
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 border-b last:border-0 text-sm">
      ...
      {task.is_comday || task.task_type === "libur_pengganti" ? (
        <span className="text-xs bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 rounded-full whitespace-nowrap">
          🏖️ Libur
        </span>
      ) : task.description ? (
