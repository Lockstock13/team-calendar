...
const TASK_TYPES = [
  {
    ...
    border:
      "border-blue-300 bg-blue-50 text-blue-700 dark:border-blue-700 dark:bg-blue-500/15 dark:text-blue-400",
  ...
  },
  {
    ...
    border:
      "border-emerald-300 bg-emerald-50 text-emerald-700 dark:border-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-400",
...
export default function TaskForm({
  ...
  const handleSubmit = (e) => {
    ...
    if (data.assignee_ids.length === 0) {
      setAssigneeError("Pilih minimal 1 fotografer.");
      return;
    }
    setAssigneeError("");
    onSubmit(data);
  ...
  return (
    ...
    >
      <div
        className="bg-background rounded-2xl border shadow-xl w-full max-w-lg max-h-[80vh] sm:max-h-[92vh] overflow-y-auto"
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-5">
          ...
          <div className="space-y-2">
            ...
            {assigneeError && (
              <p className="text-xs text-red-500 font-medium">{assigneeError}</p>
            )}
            <div className="border rounded-xl overflow-hidden divide-y max-h-64 overflow-y-auto overscroll-contain">
              {users.length === 0 && (
              ...
              {users.map((user) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left flex items-center gap-3 px-4 py-3 sm:py-2.5 cursor-pointer transition-colors touch-manipulation ${selected ? "bg-primary/5" : "hover:bg-muted/50 active:bg-muted"
                      }`}
                  >
          ...
          <div className="flex gap-2 pt-2 border-t">
            ...
            <button
              type="submit"
              disabled={submitting || data.assignee_ids.length === 0}
              className="flex-1 py-2.5 bg-primary text-primary-foreground rounded-xl text-sm font-medium hover:opacity-90 transition-opacity disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {submitting ? (
                <>
