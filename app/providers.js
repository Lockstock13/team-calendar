...
export default function Providers({ children }) {
    ...
    const fetchUsers = async () => {
        ...
        if (!error && data) {
            setUsers(data.map((u) => ({ ...u, color: u.color || generateColor(u.id) })));
        }
    };
    ...
    const handleDeleteTask = async (taskId) => {
        const ok = await confirm({
            title: "Hapus Jadwal",
            message: "Apakah kamu yakin ingin menghapus jadwal ini? Tindakan ini tidak bisa dibatalkan.",
            confirmText: "Hapus",
            cancelText: "Batal"
        });
        if (!ok) return;
        ...
        try {
            if (mode === "login") {
                ...
                if (signUpData.user && !signUpData.session) {
                    setAuthError("✅ Akun berhasil dibuat! Cek email untuk konfirmasi, lalu login.");
                ...
                if (signUpData.user) {
                    await supabase.from("profiles").upsert(
                        { id: signUpData.user.id, email: data.email, full_name: data.full_name, role: "member", is_active: true },
                        { onConflict: "id" }
                    );
                }
            }
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-muted/30 p-4">
                <div className="w-full max-w-sm bg-background border rounded-2xl p-8 text-center shadow-sm space-y-4">
                    ...
                    <button
                        onClick={() => { supabase.auth.signOut(); setInactiveBlock(false); }}
    ...
    if (!session) {
        return <AuthForm onAuth={handleAuth} loading={authLoading} error={authError} />;
    }
    ...
    return (
        <GlobalContext.Provider value={{
            session, userProfile, tasks, users, setTasks, fetchTasks,
            showForm, setShowForm, editingTask, setEditingTask, handleEditTask,
            handleDeleteTask, handleUpdateTaskStatus, handleTaskSubmit,
            language, setLanguage
        }}>
            <div className="min-h-screen bg-muted/20">
                <Header session={session} userProfile={userProfile} handleLogout={handleLogout} unreadChat={unreadChat} />
                {children}
