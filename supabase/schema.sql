...
-- Migration-safe columns for older notes tables
...
-- ─── 10. AUDIT LOGS TABLE (Security / Admin Actions) ────────────────────────
...
CREATE TABLE IF NOT EXISTS public.audit_logs (
  ...
  status     TEXT NOT NULL DEFAULT 'ok'
              CHECK (status IN ('ok', 'blocked', 'error', 'partial')),
...
-- Admin can read all audit events
